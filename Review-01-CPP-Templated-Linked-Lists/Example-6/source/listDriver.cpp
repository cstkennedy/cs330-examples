 ...
 */
LinkedList<int> generateList(int n);

/**
 * Print aLinked List of integers
 *
 * @param outs output stream
 * @param toPrt list of ints to print
 */
std::ostream& operator<<(std::ostream& outs, const LinkedList<int>& toPrt);
...
int main(int argc, char** argv)
{
    ...
    LinkedList<int> randomInts = std::move(generateList(toGenerate));
    cout << randomInts;

    printHorizontalLine(cout, '*', W_WIDTH);

    LinkedList<int> randomCopy = randomInts;

    randomCopy.push_back(337);
...
std::ostream& operator<<(std::ostream& outs, const LinkedList<int>& toPrt)
{
    ...
    for (const int& theIntValue : toPrt) {
        outs << "Node # " << std::right << std::setw(4) << index
             << " - "     << std::right << std::setw(4) << theIntValue
             << "\n";
