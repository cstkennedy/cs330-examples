...
void Circle::display( std::ostream &outs ) const
{
    ...
    outs << std::left << std::setw( WIDTH_LABEL ) << "Radius"     << ": " << std::right << std::setw( WIDTH_VALUE ) << radius()     << "\n"
         << std::left << std::setw( WIDTH_LABEL ) << "Diameter"   << ": " << std::right << std::setw( WIDTH_VALUE ) << diameter()   << "\n"
